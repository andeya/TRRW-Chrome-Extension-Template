...
Fork from [awesome-chrome-extension-boilerplate](https://github.com/tjx666/awesome-chrome-extension-boilerplate)

[![test](https://github.com/andeya/trrw-chrome-extension-template/actions/workflows/test.yml/badge.svg)](https://github.com/andeya/trrw-chrome-extension-template/actions/workflows/test.yml) [![Known Vulnerabilities](https://snyk.io/test/github/andeya/trrw-chrome-extension-template/badge.svg?targetFile=package.json)](https://snyk.io/test/github/andeya/trrw-chrome-extension-template?targetFile=package.json) [![Percentage of issues still open](https://isitmaintained.com/badge/open/andeya/trrw-chrome-extension-template.svg)](http://isitmaintained.com/project/andeya/trrw-chrome-extension-template) [![PRs Welcome](https://img.shields.io/badge/PRs-welcome-brightgreen.svg?style=flat)](https://github.com/andeya/react-typescript-boilerplate/pulls)

> 一个超棒的基于 TypeScript-React & Rust-Wasm & Webpack 的 Chrome 扩展开发模板
...
- :wasm: 支持 `wasm`，使用 `wasm-pack-plugin` 将 `Rust` 代码编译成 `wasm`，并使用 `wasm-loader` 加载 `wasm`

## :package: 安装

```bash
# 克隆这个模板
git clone https://github.com/andeya/trrw-chrome-extension-template.git
...
它俩的 webpack entry 分别是 `src/options/bgfetch.ts` 和 `src/popup/bgfetch.ts`。这两个页面很相似，都只是一个普通的 web 页面，因此你可以像开发一个 react **SPA** 一样开发它们。
...
这个模板会扫描 `src/contents` 文件夹，将所有子文件夹中的 `bgfetch.ts` 或 `index.ts` 作为 `webpack entry`。
...
2. 创建一个和上面 `content script` 路径对应的文件夹 `src/contents/discuss`。`src/discuss/bgfetch.ts` 或者 `src/discuss/index.ts` 将会被视为一个 webpack entry。 `webpack` 会通过这个 `entry` 最终产出 `js/discuss.js` 这个 `chunk`。
